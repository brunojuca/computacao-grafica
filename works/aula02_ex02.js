...
function rotateCylinder() {
  ...
  // Will execute T1 and then R1 in Y
  c1.matrix.multiply(mat4.makeRotationY(angle[3])); // R1

  // Will execute T1 and then R1 in Z
...
function buildInterface() {
  ...
  var gui = new GUI();
  gui
    .add(controls, "joint1", 0, 360)
    .onChange(function () {
      controls.rotate();
    })
    .name("First Joint");
  gui
    .add(controls, "joint2", 0, 360)
    .onChange(function () {
      controls.rotate();
    })
    .name("Second Joint");
  gui
    .add(controls, "joint3", 0, 360)
    .onChange(function () {
      controls.rotate();
    })
    .name("Third Joint");
  gui
    .add(controls, "joint4", 0, 360)
    .onChange(function () {
      controls.rotate();
    })
